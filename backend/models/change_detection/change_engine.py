...
class P3ChangeDetectionEngine:
    ...
    def _compute_change_mask(self, img1: Image.Image, img2: Image.Image) -> Tuple[np.ndarray, np.ndarray]:
        ...
        rgb_max_diff = np.max(np.abs(arr1.astype(np.float32) - arr2.astype(np.float32)), axis=2)

        combined_diff = (cielab_dist * 0.7) + (rgb_max_diff * 0.3)
        blurred_diff = cv2.GaussianBlur(combined_diff.astype(np.float32), (5, 5), 0)
    ...
    def _extract_spatial_evidence(
        ...
        min_area_px = (width * height) * 0.003  # 0.3% minimum area threshold
