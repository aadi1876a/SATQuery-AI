...
def run_captioning(tool_input: ToolInput) -> ToolOutput:
    ...
    try:
        ...
        # Calculate true sequence generation confidence from beam/token softmax probabilities
        conf = None
        if hasattr(out, "sequences_scores") and out.sequences_scores is not None:
            # Beam search provides sequence log-probabilities; convert to probability
            conf = round(float(pytorch.exp(out.sequences_scores[0]).item()), 3)
